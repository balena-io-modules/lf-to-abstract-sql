(function(root, factory) {
    ...
    SBVRCompilerLibs.MappedFactType = function(factType) {
        ...
        for (var i = 0; traverseInfo.__valid.length > i; i++) mappedFactType[i] = traverseInfo.__valid[i].slice();
    ...
    SBVRCompilerLibs.UnmappedFactType = function(factType) {
        ...
        for (var i = 0; mappedFactType.length > i; i++) mappedFactType[i] = mappedFactType[i].slice(0, 3);
        return mappedFactType;
    };
    ...
    SBVRCompilerLibs.GetTableFieldID = function(table, fieldName) {
        for (var tableFields = table.fields, i = 0; tableFields.length > i; i++) if (tableFields[i].fieldName == fieldName) return i;
        return !1;
    };
});