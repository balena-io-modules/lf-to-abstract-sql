...
const realType = term('Real', 'Type');
const dateType = term('Date', 'Type');
const lengthType = term('Length', 'Type');

const has = verb('has');
const isOf = verb('is of');
const name = term('name');
const locale = term('locale');
const language = term('language');
const region = term('region');
const country = term('country');
const latitude = term('latitude');
const longitude = term('longitude');
const city = term('city');
const media = term('media');
const tourType = term('tour type');
const mediaType = term('media type');
const comment = term('comment');
const durationInDays = term('duration in days');
const plan = term('plan');
const tour = term('tour');
...
const application = term('application');
const itinerary = term('itinerary');
const overnights = term('overnights');
const order = term('order');
const dayNumber = term('day number');
const location = term('location');
const story = term('story');
const recommendation = term('recommendation');
const link = term('link');
const banner = term('banner');
const requirement = term('requirement');
const firstName = term('first name');
const lastName = term('last name');
const email = term('email');
const subscriber = term('subscriber');
const recipientGroup = term('recipient group');
const service = term('service');
const option = term('option');
const site = term('site');
const value = term('value');
const page = term('page');
const body = term('body');
const url = term('url');
const linkCategory = term('link category');
const width = term('width');
const height = term('height');

const regionTranslation = term('region translation');
const countryTranslation = term('country translation');
const cityTranslation = term('city translation');
const mediaTranslation = term('media translation');
const tourTypeTranslation = term('tour type translation');
const tourTranslation = term('tour translation');
const applicationTranslation = term('application translation');
const storyTranslation = term('story translation');
const requirementTranslation = term('requirement translation');
const recommendationTranslation = term('recommendation translation');
const linkTranslation = term('link translation');
const bannerTranslation = term('banner translation');
const serviceInclusion = term('service inclusion');
const serviceExclusion = term('service exclusion');
const siteOption = term('site option');
const pageTranslation = term('page translation');

describe('vforvacation', function () {
	// Term:	   name
	test(Table(name));
	...
	test(
		attribute(
			necessity('each', locale, has, 'a', [lengthType, verb('is equal to'), 2]),
		),
	);
	...
	test(
		attribute(necessity('each', locale, isOf, ['exactly', 'one'], language)),
	);
	...
	test(
		attribute(
			necessity('each', regionTranslation, has, ['exactly', 'one'], name),
		),
	);
	...
	test(
		attribute(
			necessity('each', countryTranslation, has, ['exactly', 'one'], name),
		),
	);
	...
	test(
		attribute(
			necessity('each', cityTranslation, has, ['exactly', 'one'], name),
		),
	);
	...
	test(
		attribute(
			necessity('each', description, has, 'a', [
				lengthType,
				verb('is less than or equal to'),
				2000,
			]),
	...
	test(
		attribute(
			necessity('each', mediaTranslation, has, ['exactly', 'one'], title),
		),
	);
	...
	test(
		attribute(
			necessity('each', mediaTranslation, has, ['exactly', 'one'], description),
		),
	);
	...
	test(
		attribute(
			necessity('each', tourTypeTranslation, has, ['exactly', 'one'], name),
		),
	);
	...
	test(
		attribute(
			necessity('each', tourTranslation, has, ['exactly', 'one'], title),
		),
	);
	...
	test(
		attribute(
			necessity('each', tourTranslation, has, ['exactly', 'one'], description),
		),
	);
	...
	test(
		attribute(
			necessity('each', tourTranslation, has, ['exactly', 'one'], comment),
		),
	);
	...
	test(
		attribute(necessity('each', plan, has, ['exactly', 'one'], durationInDays)),
	);
	...
	test(
		attribute(
			necessity('each', application, has, ['exactly', 'one'], departureDate),
		),
	);
	...
	test(
		attribute(
			necessity('each', application, has, ['exactly', 'one'], singlePrice),
		),
	);
	...
	test(
		attribute(
			necessity('each', application, has, ['exactly', 'one'], doublePrice),
		),
	);
	...
	test(
		attribute(necessity('each', application, has, ['exactly', 'one'], airFee)),
	);
	...
	test(
		attribute(necessity('each', application, isOf, ['exactly', 'one'], plan)),
	);
	// 	Necessity: each plan has at least one application.
	test(
		attribute(necessity('each', plan, has, ['at least', 'one'], application)),
	);
	...
	test(
		attribute(
			necessity('each', storyTranslation, has, ['exactly', 'one'], location),
		),
	);
	...
	test(
		attribute(
			necessity('each', storyTranslation, has, ['exactly', 'one'], description),
		),
	);
	...
	test(
		attribute(necessity('each', requirement, isOf, ['exactly', 'one'], tour)),
	);
	...
	test(
		attribute(
			necessity('each', recommendation, isOf, ['exactly', 'one'], tour),
		),
	);
	...
	test(
		attribute(necessity('each', linkCategory, has, ['exactly', 'one'], name)),
	);
	...
	test(
		attribute(
			necessity('each', linkTranslation, has, ['exactly', 'one'], title),
		),
	);
	...
	test(
		attribute(
			necessity('each', bannerTranslation, has, ['exactly', 'one'], title),
		),
	);
	...
	test(
		attribute(
			necessity('each', firstName, has, 'a', [
				lengthType,
				verb('is less than or equal to'),
				100,
			]),
	...
	test(
		attribute(
			necessity('each', subscriber, has, ['at most', 'one'], firstName),
		),
	);
	...
	test(
		attribute(necessity('each', subscriber, has, ['at most', 'one'], lastName)),
	);
	// Fact type: subscriber has email
	test(Table(factType(subscriber, has, email)));
	// 	Necessity: each subscriber has exactly one email.
	test(
		attribute(necessity('each', subscriber, has, ['exactly', 'one'], email)),
	);
	...
	test(
		attribute(necessity('each', recipientGroup, has, ['exactly', 'one'], name)),
	);
	...
	test(
		attribute(necessity('each', service, has, ['exactly', 'one'], language)),
	);
	...
	test(
		attribute(
			necessity('each', serviceInclusion, has, ['exactly', 'one'], order),
		),
	);
	...
	test(
		attribute(
			necessity('each', serviceInclusion, has, ['exactly', 'one'], description),
		),
	);
	...
	test(
		attribute(
			necessity('each', serviceExclusion, has, ['exactly', 'one'], order),
		),
	);
	...
	test(
		attribute(
			necessity('each', serviceExclusion, has, ['exactly', 'one'], description),
		),
	);
	...
	test(
		attribute(necessity('each', siteOption, has, ['exactly', 'one'], value)),
	);
	...
	test(
		attribute(
			necessity('each', pageTranslation, has, ['exactly', 'one'], title),
		),
	);
	...
	test(
		attribute(
			necessity('each', pageTranslation, has, ['exactly', 'one'], body),
		),
	);
});
